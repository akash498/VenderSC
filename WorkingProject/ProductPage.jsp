...
<!-- End of Header Div -->
	...
	<%
		...
		if(request.getParameter("prodNum")!=null){
			index = (Integer.parseInt(request.getParameter("prodNum")));
			for(int i=0; i < products.size(); i++){
				if(products.get(i).getProductId()==index){
					p=products.get(i);
				}
			}
		}
		else if(request.getParameter("prodIndex")!=null){
			index = (Integer.parseInt(request.getParameter("prodIndex")));
	...
	<div id="containerDiv">
		...
		<div id="imageDiv">
			<img src="<%= p.getImagePath() %>" alt="logo" id = "logo">
		</div>
		...
		<div id="productDiv">
			...
			<% if(session.getAttribute("user")!=null) { %>
				<form action="ProductPageServlet" method="GET">
					<input type="hidden" name="prodIndex" value="<%= p.getProductId() %>" />
	...
	<div id="reviewDiv">
		...
		<% 
			if(session.getAttribute("user")!=null && ((User)session.getAttribute("user")).hasPurchased(p)) { 
